...
def GetArgs():
   ...
   parser.add_argument('-n', '--vmname', required=True, action='append', help='Names of the Virtual Machines to power')
   parser.add_argument('-a', '--action', required=True, action='store', help='Action of the Virtual Machines to process: on off reset suspend')
...
def main():
   ...
   try:
      ...
      if args.action == 'on':
        ...
        tasks = [vm.Suspend() for vm in vmList if vm.name in vmnames]
      else:
        print("bad action: on off reset")
        return
