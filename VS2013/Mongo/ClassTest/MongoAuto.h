...
namespace MongoClib
{
	...
	public:
		...
		AutoBson& operator=(AutoBson&& rhs) { m_p = rhs.Release(); }
		...
		bool Add(const std::string& key, const std::wstring& val)
			{ return BSON_APPEND_UTF8(m_p, key.c_str(), EncodeU8(val).c_str()); }
	...
	public:
		...
		// return false if record not exist.
		bool Modify(bson_t* query, bson_t* update, bool bMulti = false);
		bool ModifyMulti(bson_t* query, bson_t* update)
			{ return Modify(query, update, true); };
	...
	private:
		...
		void SetErr(const std::string& str)
		{
			...
			_memccpy(m_err.message, str.c_str(), 0, len);
