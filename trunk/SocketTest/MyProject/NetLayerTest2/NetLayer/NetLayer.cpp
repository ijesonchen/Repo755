...
void LinkMap::Add(SOCKET sd, CSharePtr<NetLinkTask>& spTask)
{
	CSingleLock lock(&csLink, true);
	mapLinks.insert(pair<SOCKET, CSharePtr<NetLinkTask>>(sd, spTask));
}

void LinkMap::Remove(SOCKET sd)
{
	CSingleLock lock(&csLink, true);
	mapLinks.erase(sd);
}
