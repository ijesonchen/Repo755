...
const bool NL_CLIENT_IS_SERVER = false;

NetLongClient::NetLongClient(const wchar_t* pAddr, const wchar_t* pSysId, void* pvThreadPool)
: NetBase(NL_CLIENT_IS_SERVER, pvThreadPool)
, m_addrUnicode(pAddr)
, m_strSysid(pSysId)
...
{
	wstringstream ss;
	ss << L"NetLongClient-" << pAddr;
	m_msg.SetPrefix(ss.str().c_str());
}

NetLongClient::~NetLongClient(void)
{
	NotifyThreadToExit();
	WaitThreadToExit();
	CleanSocket();
...
bool NetLongClient::QueueEchoMsg(void)
{
	EchoReqRes echo;
...
bool NetLongClient::InitLayer(void)
{
...
SOCKET  NetLongClient::TryConnect(void)
{
	...
	// connect
	SOCKET sd = TryConnectServer(m_addrUnicode.c_str(), m_uRemotePort);
...
void NetLongClient::CleanSocket(void)
{
	if (m_socket != INVALID_SOCKET)
	{
...
void NetLongClient::OnConnect(SOCKET connSock)
{
	NetLinkTask* pTask;// = new NetLinkTask(this, connSock, true);
	ThreadPoolPtr pool = (ThreadPoolPtr)m_pvThreadPool;
	pool->QueueRequest(pTask);
}

void NetLongClient::OnNetFailure(void)
{
	CleanSocket();
}

bool NetLongClient::FailWhenNoRecv(void)
{
	return time(NULL) - m_tmLastRecv >= m_nRecvTimeOutSecs;
}

bool NetLongClient::OnRecvMsg(google::protobuf::int64 sn, const int nType, 
...
void NetLongClient::OnNoDataSend(void)
{
...
void NetLongClient::OnSend(SOCKET sd, MsgBuffer* const pMsgBuf )
{
